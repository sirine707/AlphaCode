...
import type { ActiveView, ActiveRightView } from "./code-canvas-layout";
...
interface ActivityBarProps {
  ...
  onViewChange: (viewId: ActiveView | ActiveRightView) => void;
...
const ActivityBar: React.FC<ActivityBarProps> = ({
  ...
  const handleItemClick = (itemId: ActiveView | ActiveRightView) => {
    onViewChange(itemId);
  };

  const getIconForItem = (item: ActivityBarItem) => {
    if (item.id === "explorer") {
      return isSidePanelOpen && activeViewId === "explorer" ? (
    ...
    if (item.id === "deploy") {
      return isSidePanelOpen && activeRightViewId === "deploy" ? (
        ...
        <div className="flex flex-col space-y-2">
          {navItems.map((item) => (
            <Tooltip key={item.id}>
              <TooltipTrigger asChild>
                <Button
                  ...
                  className={cn(
                    ...
                    activeRightViewId === item.id &&
                      "text-primary bg-primary/20",
                    activeViewId === item.id &&
                      !isSidePanelOpen &&
                      "text-primary" // Highlight if active but panel closed
                  )}
                  onClick={() => handleItemClick(item.id)}
                  aria-label={item.name}
                  aria-pressed={
                    (activeViewId === item.id && isSidePanelOpen) ||
                    activeRightViewId === item.id
                  }
                >
                  {getIconForItem(item)}
              ...
              <TooltipContent
                side="right"
                className="bg-card text-foreground border-border shadow-xl"
              >
                <p>{item.name}</p>
              </TooltipContent>
            </Tooltip>
