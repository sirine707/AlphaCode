...
const SettingsPanel: React.FC<SettingsPanelProps> = ({ isOpen }) => {
  ...
  const [openSections, setOpenSections] = useState<Record<SettingsSectionName, boolean>>({
    appearance: true,
    editor: true,
    terminal: true,
  });
          ...
          <div className="flex flex-col h-full space-y-3">
            ...
            <Card className="shadow-none border-border/50">
              <CardHeader
                ...
                role="button" tabIndex={0} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleSection('appearance')}
                aria-expanded={openSections.appearance} aria-controls="appearance-content"
              >
                <CardTitle className="text-sm font-medium">Appearance</CardTitle>
                {openSections.appearance ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
              </CardHeader>
              {openSections.appearance && (
                <CardContent className="space-y-4 p-3 pt-0" id="appearance-content">
            ...
            <Card className="shadow-none border-border/50">
              <CardHeader
                ...
                role="button" tabIndex={0} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleSection('editor')}
                aria-expanded={openSections.editor} aria-controls="editor-content"
              >
                <CardTitle className="text-sm font-medium">Editor</CardTitle>
                {openSections.editor ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
              </CardHeader>
              {openSections.editor && (
            ...
            <Card className="shadow-none border-border/50">
              <CardHeader
                ...
                role="button" tabIndex={0} onKeyDown={(e) => (e.key === 'Enter' || e.key === ' ') && toggleSection('terminal')}
                aria-expanded={openSections.terminal} aria-controls="terminal-content"
              >
                <CardTitle className="text-sm font-medium">Terminal</CardTitle>
                {openSections.terminal ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
              </CardHeader>
              {openSections.terminal && (
                <CardContent className="space-y-4 p-3 pt-0" id="terminal-content">
                   <div className="space-y-1">
